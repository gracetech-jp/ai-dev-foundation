...
#
# 共通リポで改善・追加された共通所有ファイルを、各サービスが自分のタイミングで取り込む。
# 配布は一方通行（共通リポ → サービス）。逆輸入（サービス → 共通）は 2026-07-24 に廃止した
# （ADR-009。共通所有ファイルはサービス側で編集せず、改善は共通リポで直接行う）:
#
...
#
# 設計上の判断:
#   - マニフェストは【共通リポ側】の .backport-manifest を正とする（名称は互換のため維持）。
#     サービス側ローカルのマニフェスト改変で取込範囲が歪むのを防ぐため（正本は1つ・ADR-003）。
#   - 新規ファイルは既定で追加する。新規＝共通リポで生まれた新ルールであり、
...
if git -C "$ROOT" rev-parse --git-dir >/dev/null 2>&1; then
	...
	if [ -n "$dirty" ] && [ "$FORCE" -ne 1 ]; then
		...
		echo "$dirty" | sed 's/^/      /' >&2
		echo "   先に commit / stash してください（共通所有ファイルはサービス側で編集しない原則。ADR-009。意図的に無視するなら --force）。" >&2
		exit 1
	fi
fi
