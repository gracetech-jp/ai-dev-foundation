...
# 参照方式（共通リポの common/scripts/ から各プロジェクトを検証する）では、スクリプトの位置と
# 検証対象が別リポジトリになるため、位置からの導出では正しいルートに解決できない。
# 引数を任意にしてあるのは、既存の呼び出し（引数なし）を壊さないため（2026-07-26 フェーズ0）。
...
# ---- 差分の基準を決める ----
EMPTY_TREE="$(git -C "$ROOT" hash-object -t tree /dev/null 2>/dev/null || echo 4b825dc642cb6eb9a060e54bf8d69288fbee4904)"
BASE="${TRIPWIRE_BASE:-}"
if [ -z "$BASE" ]; then
	if   git -C "$ROOT" rev-parse --verify -q origin/main >/dev/null 2>&1; then BASE="origin/main"
	elif git -C "$ROOT" rev-parse --verify -q "HEAD~1"    >/dev/null 2>&1; then BASE="HEAD~1"
	else BASE="$EMPTY_TREE"; fi
fi
diff_names()      { if [ "$BASE" = "$EMPTY_TREE" ]; then git -C "$ROOT" diff --name-only "$BASE" HEAD; else git -C "$ROOT" diff --name-only "$BASE...HEAD"; fi; }
diff_hunks_file() { if [ "$BASE" = "$EMPTY_TREE" ]; then git -C "$ROOT" diff --unified=0 "$BASE" HEAD -- "$1"; else git -C "$ROOT" diff --unified=0 "$BASE...HEAD" -- "$1"; fi; }
...
if [ "$s_req_count" -eq 0 ]; then
	...
	if [ "$static_hit" -eq 0 ] && [ -n "$SENS_SYMS" ]; then
		git -C "$ROOT" grep -lE "$SENS_SYMS" -- ':!docs' >/dev/null 2>&1 && static_hit=1
	fi
