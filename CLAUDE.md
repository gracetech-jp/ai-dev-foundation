...
- 各ルールの**正本は1箇所のみ**（同じ内容をCLAUDE.md・SERVICE.md・docs/rules/に重ねて書かない）。ただしCLAUDE.mdは常時コンテキストに載る唯一のファイルで、`docs/rules/`はインデックスしか注入されない（`session-start-rules.sh`）ため、**CLAUDE.md側に各領域1行の要点＋「詳細: `docs/rules/xxx.md`」を置くのは重複ではなく設計**とする。境界は「要点＝守るべき結論を1行」「詳細＝理由・手順・例外・具体値」。`docs/rules/`の本文を逐語で写すのは禁止（2箇所が別々に育って乖離しても誰も気づけないため。2026-07-25 明文化）
