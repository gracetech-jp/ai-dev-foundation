...
| # | 未確認事項 | 確認方法 |
|---|---|---|
| 1 | 基盤 CI 6ジョブの直近実行結果・実行時間 | `gh run list --workflow=CI --limit 5`（要 gh 導入）または GitHub Actions UI。※未コミット52件が push されるまで今回変更分の CI は未発火 |
| 2 | secret-scan（gitleaks）のローカル動作 | docker 導入環境で ci.yml:61 のコマンドを手動実行 |
| 3 | 配布 CI（_base ci.yml の gates 統合・soft 契約）の実発火 | サービスリポ（grace-tech-hp 等）の Actions で TODO ターゲットが warning・実装済みが red になることを確認 |
| 4 | R-002 の deny（ツール層）がサービス側 Claude Code セッションで効くこと | サービスリポで新セッションを開き Edit で CLAUDE.md を編集→拒否されることを確認（guard 側の bash 遮断は本日実地確認済み） |
| 5 | devcontainers/ci@v0.3 のビルド成功（プロファイル生成物） | 生成サービスを push して gates ジョブのビルドログを確認 |
