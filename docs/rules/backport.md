...
- **profiles/ 由来の骨格（`.claude/` 配下・`.devcontainer/`・`.github/workflows/`・`Makefile`・監査雛形等）は
  マニフェスト対象外＝手動同期**。理由は、共通リポでは `profiles/_base/.claude/...` に置かれるものが
  サービスでは `.claude/...` に来るなど**パスが 1:1 対応せず、マニフェストの行指向スキーマ（`<op> <path>`）で
  表現できない**ため（`.backport-manifest` は ADR-009 でサービス側から削除済みなので、
  サービスからこの注記の原典は参照できない。ここに転記しておく）。
